...
以下の変更を試すと、Event Sourcing の理解が深まります:

1. **新しいイベントタイプを追加する**
   `events.py` に `REFUND_REQUESTED`（返金申請）などのイベントを追加し、
   `event_store.py` の fold ロジックを対応させてみましょう。

2. **スナップショットを実装する**
   イベントが 100 件を超えたら「現在の状態をそのまま Kafka に書き出す」
   スナップショット機能を追加してみましょう（再構築コストの削減）。

3. **任意時点の状態を再現する**
   `--since 2026-07-03T10:00:00` オプションで「その時刻以前のイベントだけを fold」して、
   過去のある瞬間の注文状態を表示してみましょう。

4. **kafka-ui で自分でオフセットを追う**
   `eventsourcing.orders.v1` トピックを開き、特定の注文 ID（Key）のメッセージを
   時系列で追って、イベントの蓄積をビジュアルで確認してみましょう。
